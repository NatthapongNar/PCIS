...
$(function() {
	...
	console.log(check_denied)
	// Initialize Share-Buttons
	$.contactButtons({
	  effect  : 'slide-on-scroll',
	  buttons : {
		    'Main': { class: 'facebook fg-white', use: true, icon: 'home', hint: hint_main, link: pathFixed + 'metro/appProgress' },
		    'Preview': { class: 'linkedin fg-white', use: true, icon: 'eye',  hint: hint_prev, link: pathFixed + 'metro/routers?_=' + n + '&rel=' + query['rel'] + '&req=P1&live=1&wrap=' + new Date().getTime() },
		    'Edit': { class: 'gplus fg-white', use: false, icon: 'ban', hint: hint_edit, iopt: 'fa-stack-2x', link: '#' },
		    'Save':{ class: 'git fg-white', use: saveable, icon: 'save', hint: hint_save, link: '#', extras: 'id="submitProfileForm"' }
	  }

	});
	
	$('#submitProfileForm').on('click', function() { $('#btnProfileSubmit').click(); });	
