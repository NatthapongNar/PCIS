...
class Uploader {
    ...
    public function __construct(){
        ...
        $this->re_name	  = $this->generateRandomString(20);
    }
	...
	private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                $metas['file'] = $this->options['uploadDir'] . $this->re_name . '.' . $metas['extension'];//$metas['name'];
                $metas['replaced'] = file_exists($metas['file']);
                $metas['date'] = date('r');
                $metas['re_name'] = $this->re_name . '.' . $metas['extension'];
	...
	private function generateRandomString($length = 10) {
	    ...
	    return 'REF_' . $randomString.'_'.date('dmY');
	}
